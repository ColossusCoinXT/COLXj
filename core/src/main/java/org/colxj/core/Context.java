...
import org.colxj.core.listeners.BlockChainListener;
import org.colxj.store.FlatDB;
import org.colxj.store.HashStore;
import org.colxj.store.MasternodeDB;
 ...
 * colxj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating colxj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use colxj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a colxj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using colxj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing colxj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.colxj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.colxj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.colxj.core.NetworkParameters} specified when this context was (auto) created. The
